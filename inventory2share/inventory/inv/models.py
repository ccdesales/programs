...
class ShoppingList(models.Model):
	name 	= models.CharField(max_length=30)	
	items 	= models.ManyToManyField(Item, through='ShoppingListEntry')
...
class InventoryEntry(models.Model):
	...
	def __str__(self):
		return "<%s %s %s of %s>" % (self.exist_quantity, self.reorder_quantity, self.item.unit, self.item.name)

class ShoppingListEntry(models.Model):
	item 		  = models.ForeignKey(Item)
	shopping_list = models.ForeignKey(ShoppingList)
	quantity 	  = models.PositiveIntegerField(max_length=3, null=True, blank=True)	  
	
	def __str__(self):
		return "<%s %s of %s>" % (self.quantity, self.item.unit, self.item.name)
